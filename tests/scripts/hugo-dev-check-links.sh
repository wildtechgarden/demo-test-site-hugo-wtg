...
systemd-run -E PATH="${PATH}" -E HUGO_RESOURCEDIR="$(pwd)"/resources -E HUGO_MODULE_REPLACEMENTS="$HUGO_MODULE_REPLACEMENTS" --unit=hugo-serve --user hugo serve --source "$(pwd)/tests/config" --environment "production" --config "$(pwd)"/hugo.toml,"$(pwd)"/tests/config/demo-site.toml --port 1313 --bind 127.0.0.1
